...
# 计算图像的梯度直方图特征
def image_gradient(target_image):
    # 将图像转换为16*16
    ...
    # 将直方图归一化
    cv2.normalize(target_image_hist, target_image_hist, 0, 1, cv2.NORM_MINMAX)
    # 将直方图转换为一维向量
...
def model_gradient_test():
    ...
    # 计算图像的颜色直方图特征
    target_image_hash = image_gradient(target_image)
    ...
    for file in os.listdir('./img/picture'):
        ...
        for (x, y, window) in sliding_window(search_image, window_size, stride):
            # 计算窗口的哈希值
            window_hash = image_gradient(window)
            # 计算直方图的交集，即相似度,并保证相似度为0-1之间
            similarity = cv2.compareHist(target_image_hash, window_hash, cv2.HISTCMP_BHATTACHARYYA)
